        ...
        if (existingUser.recordset.length === 0) {
            ...
            await query(
                `INSERT INTO Users (mobile, otp_hash, otp_expires_at) 
                 VALUES (@mobile, @otpHash, @expiresAt)`,
                { mobile, otpHash, expiresAt }
            );
        } else {
            // Existing user - update OTP
            await query(
                `UPDATE Users SET otp_hash = @otpHash, otp_expires_at = @expiresAt, 
                 updated_at = SYSUTCDATETIME() WHERE mobile = @mobile`,
                { mobile, otpHash, expiresAt }
            );
        }
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        const result = await query(
            'SELECT id, mobile, name, role, otp_hash, otp_expires_at FROM Users WHERE mobile = @mobile',
            { mobile }
        );
        ...
        await query(
            ...
            { refreshToken: tokens.refreshToken, id: user.id }
        );
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        const result = await query(
            'SELECT id, mobile, name, role FROM Users WHERE id = @id AND refresh_token = @refreshToken',
            { id: decoded.id, refreshToken }
        );
        ...
        await query(
            'UPDATE Users SET refresh_token = @newToken, updated_at = SYSUTCDATETIME() WHERE id = @id',
            { newToken: tokens.refreshToken, id: user.id }
        );
    ...
    try {
        ...
        await query(
            `UPDATE Users SET 
                ...
                default_address = COALESCE(@address, default_address),
            ...
            { 
                ...
                lat: default_lat || null,
                lng: default_lng || null,
                id: req.user.id 
            }
        );
